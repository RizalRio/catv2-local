...
var getData = (function () {
  var table = function () {
    var datatable = $("#table_groups_scales").KTDatatable({
      data: {
        type: "remote",
        source: {
          read: {
      ...
      columns: [{
        ...
        },
        {
          ...
          overflow: "visible",
          template: function (row) {
            return '<a href="' + HOST_URL + '/edit/' + row.no +
              '" class="btn btn-sm btn-clean btn-icon mr-2" title="Edit">\
                  ...
                  <button data-action="delete" data-nya="' + row.no + '" class="btn btn-sm btn-clean btn-icon" title="Delete">\
    ...
    $('#modal-filter-button').on('click', function (e) {
      ...
      if (Object.keys(value).length > 0) {
        datatable.search(JSON.stringify(value), 'multiple');
        $('#filter-modal-trigger').removeClass("btn-light-info");
        $('#filter-modal-trigger').addClass("btn-info");
      } else {
        datatable.setDataSourceParam('query', null);
        datatable.load();
        $('#filter-modal-trigger').removeClass("btn-info");
