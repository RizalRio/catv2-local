...
var getData = (function () {
  var table = function () {
    var datatable = $("#table_orders").KTDatatable({
      data: {
        type: "remote",
        source: {
          read: {
      ...
      columns: [
        ...
        { field: "client", title: "Peserta Tes" },
        {
          field: "order",
          title: "Status",
          template: function (row) {
            var status = {
              0: { title: "Pesanan Baru", state: "info" },
              1: { title: "Terkonfirmasi", state: "success" },
            };
            return (
              '<span class="label label-' +
              status[row.order].state +
              ' label-dot mr-2"></span><span class="font-weight-bold text-' +
              status[row.order].state +
              '">' +
              status[row.order].title +
              "</span>"
            );
          },
        },
        {
          ...
          template: function (row) {
            var status = {
              0: { title: "Belum Terbayar", state: "warning" },
              1: { title: "Terbayar", state: "success" },
            };
            return (
              '<span class="label label-' +
              status[row.payment].state +
              ' label-dot mr-2"></span><span class="font-weight-bold text-' +
              status[row.payment].state +
              '">' +
              status[row.payment].title +
              "</span>"
            );
          },
        },
