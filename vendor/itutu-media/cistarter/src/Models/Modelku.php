...
class Modelku extends Model
{
  ...
  public function _query(array $params = [])
  {
    ...
    if (isset($params['whereIn']) && $params['whereIn'] != NULL) {
      foreach ($params['whereIn'] as $whereIn) {
        $builder->whereIn($whereIn[0], $whereIn[1]);
      }
    }
    if (isset($params['orWhereIn']) && $params['orWhereIn'] != NULL) {
      foreach ($params['orWhereIn'] as $orWhereIn) {
        $builder->orWhereIn($orWhereIn[0], $orWhereIn[1]);
      }
    }
    if (isset($params['whereNotIn']) && $params['whereNotIn'] != NULL) {
      foreach ($params['whereNotIn'] as $whereNotIn) {
        $builder->whereNotIn($whereNotIn[0], $whereNotIn[1]);
      }
    }
    if (isset($params['orWhereNotIn']) && $params['orWhereNotIn'] != NULL) {
      foreach ($params['orWhereNotIn'] as $orWhereNotIn) {
        $builder->orWhereNotIn($orWhereNotIn[0], $orWhereNotIn[1]);
      }
    }
    ...
    if (isset($params['orlike']) && $params['orlike'] != NULL) {
      foreach ($params['orlike'] as $orlike) {
        $builder->orLike($orlike[0], $orlike[1], $orlike[2]);
      }
    }
  ...
  public function baris($id, array $params = [])
  {
    $query = $this->_query($params);
  ...
  public function tambah($data)
  {
    ...
    $data[$this->createdByField] = ($data['user_id']) ?? user()->id;
  ...
  public function ubah($id, $data)
  {
    ...
    $data[$this->updatedByField] = ($data['user_id']) ?? user()->id;
  ...
  public function hapus($id)
  {
    ...
    $data[$this->deletedByField] = ($data['user_id']) ?? user()->id;
  ...
  public function pulih($id)
  {
    ...
    $data[$this->restoredByField] = ($data['user_id']) ?? user()->id;
    return $this->save($data);
  }
