...
class GlobalController extends Controller
{
	...
	public function initController(\CodeIgniter\HTTP\RequestInterface $request, \CodeIgniter\HTTP\ResponseInterface $response, \Psr\Log\LoggerInterface $logger)
	{
		...
		if (logged_in())
			$this->data['avatar'] = (user()->avatar == DEFAULT_AVATAR) ? user()->avatar : 'uploads/' . user()->username . '/' . user()->avatar;
	}
	...
	public function errorPage()
	{
		...
		if ($segments[0] == 'support') {
			...
			$this->data['breadCrumb'] = ($this->data['breadCrumb']) ?? ['Dashboard' => 'support'];
			$this->data['messages']   = ($this->data['messages']) ?? $this->im_message->get();
			$this->render('IM\CI\Views\vAError');
		} else {
			$this->render('IM\CI\Views\vPError');
		}
	}
