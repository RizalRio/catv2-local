...
class Trash extends AdminController
{
	...
	public function list()
	{
		if ($this->request->isAJAX()) {
			...
			$mMethods = new M_methods();
			$params['select'] = "id, name, 'methods' module";
			$methods  = $mMethods->sampah($params);
			...
			$mAnswers = new M_answers();
			$params['select'] = "a.id, answer name, 'answers' module";
			$answers  = $mAnswers->sampah($params);

			$mTests = new \App\Models\M_tests();
			$params['select'] = "a.id, name, 'tests' module";
			$tests  = $mTests->sampah($params);

			$mRooms = new \App\Models\M_rooms();
			$params['select'] = "id, name, 'rooms' module";
			$rooms  = $mRooms->sampah($params);
			...
			else if ($module == 'answers')
				$allData = $answers['rows'];
			else if ($module == 'tests')
				$allData = $tests['rows'];
			else if ($module == 'rooms')
				$allData = $rooms['rows'];
			else
				$allData = array_merge($methods['rows'], $dimensions['rows'], $questions['rows'], $answers['rows'], $tests['rows'], $rooms['rows']);
			$total = count($allData);
